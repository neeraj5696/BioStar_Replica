      ...
      <div className="card-header-enhanced">
        <div className="header-left">
          ...
          <div className="title-section">
            <h3 className="card-title-enhanced">Segment</h3>
            <p className="card-subtitle">Daily Plant wise Segment Analytics</p>
          </div>
        </div>
      ...
      <div className="emp-late-content">
        <div className="bar-chart-section">
          <div className="bar-grid-enhanced">
            {lateExitData.slice(0,4).map((day, i) => {
              const heightPct = Math.round((day.lateExitCount / maxCount) * 100);
              const labels = ['Welder', 'Helper', 'Operator', 'Mechanic'];
              return (
                <div key={i} style={{ marginRight: '30px' }} className="bar-column-enhanced">
                  <div className="bar-wrapper-enhanced">
                    <div className="bar-fill-enhanced" style={{ height: `${heightPct-0}%` }}>
                      <span className="bar-value-enhanced">{day.lateExitCount-2}%</span>
                    </div>
                  </div>
